...
mongoose.connect(
    "mongodb+srv://kagan:kagan@cluster0.icf29.mongodb.net/storeappnuxt?retryWrites=true&w=majority", { useNewUrlParser: true, useUnifiedTopology: true }
);
...
app.post("/add-usercheck", (req, res) => {
    ...
    UserCheckModel.find({ userId: id }, (err, docs) => {
        if (docs.length > 0) {
            UserCheckModel.findOneAndUpdate({ userId: req.body.userId }, {
                $push: {
                    cart: req.body.carts
                }
            }).then(response => {
